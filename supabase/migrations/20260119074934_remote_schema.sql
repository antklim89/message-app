...
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "public" GRANT ALL ON TABLES TO "service_role";
  ...
  create policy "Avatars delete for authors only"
  on "storage"."objects"
  as permissive
  for delete
  to authenticated
using (((bucket_id = 'avatars'::text) AND (name = (auth.uid())::text)));
  ...
  create policy "Avatars insert for authors only"
  on "storage"."objects"
  as permissive
  for insert
  to authenticated
...
using ((bucket_id = 'avatars'::text));
  ...
  create policy "Avatars update and delete for authors only"
  on "storage"."objects"
  as permissive
  for update
  to authenticated
using (((bucket_id = 'avatars'::text) AND (name = (auth.uid())::text)));
  ...
  for delete
  to authenticated
using (((bucket_id = 'gallery'::text) AND (name = (auth.uid())::text)));
  ...
  for insert
  to authenticated
with check (((bucket_id = 'gallery'::text) AND (name = (auth.uid())::text)));



  create policy "Gallery select for all"
  on "storage"."objects"
  as permissive
  for select
  to public
using ((bucket_id = 'gallery'::text));
  ...
  create policy "Gallery update for authors only"
  on "storage"."objects"
  as permissive
  for update
  to authenticated
